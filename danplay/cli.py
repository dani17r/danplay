...
def _rel(path) -> str:
    ...
    Con `relative_to` a secas, cualquier carpeta gestionada fuera de la
    biblioteca principal (un disco externo) tumbaba el informe entero.
    """
    try:
...
def _need(value, usage: str):
    """El argumento que falta, dicho en castellano y sin traza de Python."""
    if value in (None, ""):
        raise SystemExit(f"{RED}falta un dato{RESET}: {usage}")
    return value


def _playlist_id(value) -> int:
    """El id de una lista: su numero o su nombre."""
    _need(value, "danplay playlist <accion> <id o nombre>")
    try:
        return int(value)
    except ValueError:
        pl = playlists.by_name(value)
        if not pl:
            raise SystemExit(f"{RED}no hay ninguna lista «{value}»{RESET}") from None
        return pl["id"]
...
def cmd_scan(args):
    ...
    if not cs:
        print(f"{YELLOW}No hay carpetas gestionadas. Usa: danplay carpeta agregar <ruta>{RESET}")
        return
    for c in cs:
    ...
    r = library.scan(progress=lambda n, total: print(f"  ... {n}/{total}", end="\r"))
    e = library.stats_of()
    print(
        f"{GREEN}{r['total']} canciones indexadas{RESET} ({r['added_count']} nuevas)  "
        f"{e['bytes'] / 2**30:.2f} GB, {e['seconds'] / 3600:.1f} h"
    )
    print(
        f"  {e['without_artist']} sin artista, {e['analyzed_count']} analizadas, "
...
def cmd_folder(args):
    if args.action == "add":
        ok = library.add_folder(_need(args.path, "danplay folder add <ruta>"), args.label or "")
        print((GREEN + "agregada" + RESET) if ok else (RED + "no existe esa ruta" + RESET))
    elif args.action == "remove":
        library.remove_folder(_need(args.path, "danplay folder remove <ruta>"))
        print("quitada")
    else:
...
def cmd_exclude(args):
    if args.action == "add":
        library.add_exclusion(
            _need(args.pattern, "danplay exclude add <patron>"), args.kind, args.note or ""
        )
        print(f"{GREEN}excluido:{RESET} {args.pattern}")
    elif args.action == "remove":
        library.remove_exclusion(_need(args.pattern, "danplay exclude remove <patron>"))
        print("quitada")
    else:
        for e in library.list_exclusions():
            print(f"  [{e['kind']:<4}] {e['pattern']:<28} {e['note']}")
        print("\n  por defecto siempre se omiten: " + ", ".join(library.DEFAULT_EXCLUDES))


def cmd_playlist(args):
    if args.action == "create":
        made = playlists.create(
            _need((args.name or "").strip(), "danplay playlist create <nombre>")
        )
        estado = "creada" if made["created"] else "ya existia"
        print(f"{GREEN}{estado}{RESET} id={made['id']}")
    elif args.action == "remove":
        playlists.remove(_playlist_id(args.name))
        print("borrada")
    elif args.action == "show":
        for i, c in enumerate(playlists.songs(_playlist_id(args.name)), 1):
            info = "*" * (c.get("stars") or 0)
            print(f"  {i:2d}. {c['artist']:<24} {c['title'][:40]:<42} {info}")
    elif args.action == "export":
        try:
            print(playlists.export_m3u(_playlist_id(args.name)))
        except ValueError as e:
            raise SystemExit(f"{RED}{e}{RESET}") from None
    else:
        for l in playlists.list_all():
            print(
                f"  [{l['id']:2d}] {l['name']:<26} {l['n']:3d} temas  {l['seconds'] / 60:5.0f} min"
            )
...
def cmd_import(args):
    res = ingest.process_inbox(dry_run=args.dry_run, limit=args.limit, convert_mp3=args.convert)
    if not res:
...
def cmd_convert(args):
    if not convert.available():
        print(f"{RED}falta ffmpeg{RESET}")
        return
    paths = convert.candidates()
    if not paths:
        print(
            f"{GREEN}Nada que convertir{RESET}: todo es mp3 (las carpetas "
...
def cmd_youtube(args):
    if not youtube.available():
        print(f"{RED}{youtube.unavailable_reason()}{RESET}")
        return 1
...
def cmd_search(args):
    ...
    for f in rows:
        ...
        print(
            f"  {fav}{info} {CYAN}{f['artist'][:22]:<22}{RESET} {f['title'][:40]:<42}"
...
def cmd_duplicates(args):
    ...
    if ident:
        ...
        for g in ident:
            ...
            for r in sorted(g):
                print(f"   {_rel(r)}")
    similar = duplicates.same_song(paths, config.DUPLICATE_THRESHOLD)
    if similar:
        ...
        for g in similar:
            print()
            for r in sorted(g):
                print(f"   [{_mmss(E.duration(r))} {E.bitrate(r) // 1000:>3}k] {_rel(r)}")
    print(f"\n{len(ident)} grupos identicos, {len(similar)} grupos parecidos")
    if not args.apply:
...
def cmd_organize(args):
    ...
    if not folder.is_dir():
        print(f"{RED}No existe: {folder}{RESET}")
        return
    vocab = names.vocabulary(config.ARTISTS_DIR)
    files = [
        p for p in sorted(folder.iterdir()) if p.is_file() and p.suffix.lower() in config.EXTENSIONS
    ]
    for p in files:
        ...
        if args.apply:
            ...
            p.rename(target)
    print("\n(simulacion; usa --apply para hacerlo de verdad)" if not args.apply else "\nAplicado.")
...
def cmd_watch(args):
    ...
    try:
        ...
        raise SystemExit(f"No puedo vigilar la Entrada: {e}") from e
    ...
    class H(FileSystemEventHandler):
        def on_created(self, event):
            ...
            r = ingest.process(p)
            status = (
                GREEN + "OK" + RESET if r.action == "moved" else YELLOW + r.action.upper() + RESET
            )
            print(f"{status} {_rel(r.target) if r.target else p.name}")
...
def main(argv=None):
    ...
    y.add_argument("-n", "--results", type=int, default=5, help="cuantos traer si es una busqueda")
    y.add_argument(
        "-l", "--list", dest="list_all", action="store_true", help="solo enseñar que se bajaria"
    )
    y.add_argument(
        "--only-download",
        action="store_true",
    ...
    fo.add_argument("action", nargs="?", default="list", choices=["list", "add", "remove"])
    fo.add_argument("path", nargs="?")
    fo.add_argument("--label")
    fo.set_defaults(f=cmd_folder)

    ex = sub.add_parser("exclude", help="carpetas a omitir al indexar")
    ex.add_argument("action", nargs="?", default="list", choices=["list", "add", "remove"])
    ...
    pl.add_argument(
        "action", nargs="?", default="list", choices=["list", "create", "remove", "show", "export"]
    )
    ...
    d.set_defaults(f=cmd_duplicates)
    ...
    sv.set_defaults(
        f=lambda a: __import__("danplay.api", fromlist=["api"]).serve(a.host, a.port, a.uds)
    )

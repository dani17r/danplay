...
describe('la respuesta en vivo', () => {
  it('enseña el texto segun llega y las herramientas segun terminan, y al final queda el mensaje', async () => {
    ...
    held.polls = [
      {
        text: '',
        tools: [{ name: 'search_songs', summary: '3 resultados' }],
        done: false,
        result: null
      },
      {
      ...
      },
      {
        ...
        result: {
          ...
          actions: [],
          confirm: null,
          usage: { calls: 2, prompt: 3100, completion: 120, cost: 0.0021 },
          via: { id: 'p', name: 'P', model: 'm', fallback: false }
        }
      }
    ]
  ...
  it('parar corta la respuesta y lo que habia llegado se queda señalado', async () => {
    ...
    held.polls = [
      { text: 'Voy a', tools: [], done: false, result: null },
      {
        ...
        result: { text: '', canceled: true, tools: [], actions: [], confirm: null }
      }
    ]
    const w = await montar()
    ...
    held.polls = [
      {
        ...
        result: {
          ...
          actions: [],
          confirm: null,
          usage: { calls: 1, prompt: 10, completion: 5, cost: 0 },
          via: { id: 'llm7', name: 'LLM7', model: 'x', fallback: true }
        }
      }
    ]
    const w = await montar()
...
describe('las conversaciones guardadas', () => {
  ...
  it('al volver se abre la ultima, y desde la lista se cambia, se crea y se borra', async () => {
    held.state.chats = [
      {
        ...
        updated: 2,
        messages: [
          { role: 'me', text: 'domingo' },
          { role: 'ai', text: 'Aqui va el set.' }
        ]
      },
      {
        id: 1,
    ...
    expect(items.map((i) => i.find('.chat-item-title').text())).toEqual([
      'Set del domingo',
      'Tonos'
    ])
    ...
    held.state.chats = [
      {
        ...
        updated: 1,
        messages: [
          { role: 'me', text: '¿en que tono esta Mi Gozo?' },
          { role: 'ai', text: 'En Sol.' }
        ]
      },
  ...
  it('lo que habia en el localStorage de antes pasa a la base una sola vez', async () => {
    localStorage.setItem(
      ...
      JSON.stringify([
        { role: 'me', text: 'vieja' },
        { role: 'ai', text: 'respuesta vieja', tools: [] }
      ])
    )
    const w = await montar()
    expect(held.state.chats).toHaveLength(1)
...
describe('exportar y presupuesto', () => {
  it('copia la conversacion como texto al portapapeles', async () => {
    ...
    const w = await montar()
    const copiar = w
      .findAll('.chat-head .btn')
      .find((b) => b.attributes('title')?.includes('Copiar'))
    ...
    held.polls = [
      {
        ...
        result: {
          ...
          actions: [],
          confirm: null,
          budget: { limit: 1, month: 1.5, over: true }
        }
      }
    ]
    const w = await montar()

...
const held = vi.hoisted(() => ({ playback: null, projection: null, songs: {}, changed: null }))
...
vi.mock('../src/api.js', async (importOriginal) => {
  ...
  held.projection = {
    show: v.fn(async () => {}),
    hide: v.fn(async () => {}),
    fullscreen: v.fn(async () => {}),
    isFullscreen: v.fn(async () => false)
  }
  return {
    ...actual,
    inTauri: true,
    ...
    // «algo cambio en el nucleo»: lo manda Rust (o el sondeo del navegador)
    core: {
      onStatus: v.fn(async () => () => {}),
      onChanged: v.fn(async (fn) => {
        held.changed = fn
        return () => {
          held.changed = null
        }
...
describe('la proyeccion', () => {
  ...
  it('con letra con tiempos resalta la linea que suena y las de alrededor', async () => {
    ...
    await w
      .findAll('.proj-bar .btn')
      .find((b) => b.text() === '1 línea')
      .trigger('click')
  ...
  it('sin tiempos se pasa por bloques con las flechas', async () => {
    ...
    held.playback.emit(
      state({
        track: { id: 8, title: 'Sin tiempos', artist: 'Barak', duration: 200 },
        playing: true,
        position: 5
      })
    )
  ...
  it('una cancion sin letra lo dice, y F pide pantalla completa', async () => {
    const w = await montar()
    held.playback.emit(
      state({ track: { id: 9, title: 'Muda', artist: 'Barak', duration: 100 }, playing: true })
    )
    await flushPromises()
    await flushPromises()
...
describe('la proyeccion se entera de lo que cambia', () => {
  it('la letra que se encuentra con la cancion ya sonando llega al proyector', async () => {
    // Solo se releia al cambiar de cancion: si se buscaba la letra mientras
    // sonaba, el proyector seguia diciendo que no tenia.
    const w = await montar()
    held.playback.emit(
      state({ track: { id: 9, title: 'Muda', artist: 'Barak', duration: 100 }, playing: true })
    )
    ...
    await flushPromises()
    expect(w.text()).toContain('no tiene letra')
    held.songs[9] = { ...held.songs[9], lyrics: 'Ahora si\ntiene letra' }
    expect(held.changed, 'no escucha los cambios del nucleo').toBeTypeOf('function')
    held.changed({ revision: 4 })
    await flushPromises()
    await flushPromises()
    expect(w.find('.proj-text').text()).toContain('Ahora si')
    w.unmount()
    expect(held.changed, 'sigue escuchando despues de cerrarse').toBe(null)
  })
})

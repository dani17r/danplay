...
const uid = useId()
const ids = { label: `${uid}-label`, list: `${uid}-list` }
...
function price(m) {
  ...
  const f = (v) =>
    v == null
      ...
      : v === 0
        ? 'gratis'
        : '$' + (v < 1 ? v.toFixed(2).replace(/0$/, '') : v.toFixed(v >= 10 ? 0 : 1))
  return `${f(m.cost_in)} → ${f(m.cost_out)}`
}
function context(m) {
  if (!m.context) return ''
  return m.context >= 1e6
...
function onKey(e) {
  ...
  if (e.key === 'Enter' && open.value && shown.value[highlighted.value]) {
    e.preventDefault()
    choose(shown.value[highlighted.value])
  }
}
...
<template>
  <div ref="root" class="field model-picker" :class="{ off: disabled, open }">
    <span v-if="label || warn" :id="ids.label" class="field-label"
      >{{ label }}
    ...
    <span class="field-box">
      ...
      <input
        ...
        :aria-activedescendant="open && shown[highlighted] ? optionId(highlighted) : undefined"
        @input="type"
        @focus="open = true"
        @keydown="onKey"
      />
      ...
      <div v-if="open" class="model-menu">
        <div class="model-filters">
          <label class="chip" :class="{ on: onlyTools }">
            <input v-model="onlyTools" type="checkbox" /> con herramientas</label
          >
          <label class="chip" :class="{ on: hideOld }">
            <input v-model="hideOld" type="checkbox" /> sin obsoletos</label
          >
        ...
        <div v-if="!models.length" class="model-empty">
          {{
            loading
              ? 'buscando modelos…'
              : 'sin lista: escribe el nombre del modelo tal cual lo llama el proveedor'
          }}
        </div>
        <div v-else-if="!shown.length" class="model-empty">
          nada coincide con lo escrito
          <span v-if="onlyTools || hideOld"> (prueba quitando los filtros)</span>
        </div>
        <!-- el teclado mueve el resaltado desde la caja de texto; el raton, al
             pasar; y si una opcion llega a tener el foco, tambien -->
        <div
          v-else
          :id="ids.list"
          class="model-list"
        ...
        >
          <button
            ...
            :class="{ hilite: i === highlighted, current: m.id === model, old: m.deprecated }"
            @click="choose(m)"
          ...
          >
            <span class="model-main">
              ...
              <span class="model-badges">
                ...
                <span
                  v-if="m.trains"
                  class="tag bad"
                  title="El proveedor avisa de que puede entrenar con lo que le mandes"
                  >entrena con tus datos</span
                >
            ...
            <span class="model-sub mono">
              ...
              <span v-if="price(m)" :title="'por millón de tokens: entrada → salida'">{{
                price(m)
              }}</span>

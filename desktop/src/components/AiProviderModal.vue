...
import { ask, useDialog } from '../composables/useDialog.js'
...
// Mientras esta abierto, Tab no se va a lo de detras y al cerrar el foco
// vuelve al boton que lo abrio.
useFocusTrap(box, { active: () => props.open, initial: 'input' })
...
function keys(e) {
  ...
  // Con una confirmacion encima («¿quitar este proveedor?»), Escape es de
  // ella: cerraba las dos ventanas de golpe.
  if (dialog.value.open || e.defaultPrevented) return
  e.preventDefault()
  if (step.value === 'form') back()
  else emit('close')
}

async function load(refresh = false) {
  try {
    data.value = await api.aiProviders(refresh)
  } catch (e) {
...
const groups = computed(() => {
  ...
  return data.value.groups
    .map((g) => ({
      ...g,
      items: data.value.catalog.filter(
        (p) =>
          p.group === g.id &&
...
function pick(entry, existingId = '') {
  ...
  f.headersText = Object.entries(saved?.headers || {})
    .map(([k, v]) => `${k}: ${v}`)
    .join('\n')
...
const savedProfile = computed(() =>
  profileId.value ? data.value?.profiles?.[profileId.value] : null
)
const needsKey = computed(() => chosen.value?.key === 'required')
const urlEditable = computed(
  () =>
    !!chosen.value &&
    (chosen.value.group === 'local' || chosen.value.id === 'custom' || !chosen.value.fields.length)
)
/** La URL con los huecos ({region}, {resource}…) rellenos con lo escrito. */
const shownUrl = computed(() =>
  (form.value.base_url || '').replace(
    /\{(\w+)\}/g,
    (_, k) => form.value.fields[k]?.trim() || `{${k}}`
  )
)
...
function draft() {
  ...
  const d = {
    ...
    provider: chosen.value.id,
    model: f.model.trim(),
    chat_model: f.chat_model.trim(),
    fields: { ...f.fields },
    timeout: Number(f.timeout) || 60
  }
  if (f.key.trim()) d.key = f.key.trim()
  if (f.base_url.trim() && f.base_url.trim() !== chosen.value.base_url)
    d.base_url = f.base_url.trim()
  if (chosen.value.id === 'custom') {
    d.base_url = f.base_url.trim()
    d.name = f.name.trim() || f.fields.name || ''
  }
...
const missing = computed(() => {
  ...
  for (const x of chosen.value.fields)
    if (x.name !== 'name' && !f.fields[x.name]?.trim()) return `falta ${x.label.toLowerCase()}`
  if (needsKey.value && !f.key.trim() && !savedProfile.value?.has_key) return 'falta la clave'
  if (chosen.value.id === 'custom' && !/^https?:\/\//.test(f.base_url.trim()))
    return 'falta la URL (http o https)'
...
const modelList = computed(
  () => (models.value.ok ? models.value.models : models.value.catalog) || []
)
const listNote = computed(() => {
  ...
  if (m.catalog?.length)
    return (
      `${m.catalog.length} modelos según el catálogo` +
      (m.reason
        ? ` · el proveedor no respondió: ${m.reason}`
        : ' · con la clave, se pide la lista real')
    )
...
async function check() {
  ...
  error.value = ''
  try {
    checkResult.value = await api.aiCheck(draft())
  } catch (e) {
    checkResult.value = { ok: false, reason: errorMessage(e) }
  } finally {
...
<template>
  <transition name="fade">
    <div
      ...
      class="modal-back"
      role="presentation"
      @mousedown.self="!dialog.open && emit('close')"
    >
      <div
        ref="box"
        class="modal ai-modal"
        role="dialog"
        aria-modal="true"
        aria-label="Elegir la inteligencia artificial"
      >
        <!-- ============ paso 1: elegir ============ -->
        <template v-if="step === 'pick'">
          ...
          <div class="modal-msg">
            Todos hablan el mismo protocolo, así que vale cualquiera: un laboratorio, una plataforma
            como DeepInfra u OpenRouter, la nube de tu empresa, un modelo corriendo en este equipo o
            un servidor propio. Los ya configurados llevan una marca.
          </div>
          <TextField
            v-model="filter"
            width="100%"
          ...
          <div v-else class="ai-groups">
            <section v-for="g in groups" :key="g.id" class="ai-group">
              <h4>
                <Icon :n="GROUP_ICON[g.id] || 'cloud'" :t="14" /> {{ g.name }}
                <span class="ai-group-note">{{ g.note }}</span>
                <button
                  v-if="g.id === 'free'"
                  class="btn mini primary ai-free-btn"
                  type="button"
                  :disabled="tryingFree"
                  @click="tryFree"
                >
                  {{ tryingFree ? 'buscando uno que responda…' : 'Probar gratis ahora' }}
                </button>
              </h4>
              <div class="ai-grid">
                <template v-for="p in g.items" :key="p.id">
                  ...
                  <button
                    v-for="s in p.id === 'custom' ? savedFor('custom') : []"
                    :key="s.id"
                    type="button"
                    class="ai-card configured"
                    :class="{ active: data.active === s.id }"
                    @click="pick(p, s.id)"
                  >
                    <span class="ai-card-name">{{ s.provider_name || s.name }}</span>
                    <span class="ai-card-note mono">{{ s.base_url }}</span>
                    <span class="ai-card-mark"
                      ><Icon n="check" :t="12" />
                      {{ data.active === s.id ? 'en uso' : 'configurado' }}</span
                    >
                  </button>
                  <button
                    type="button"
                    class="ai-card"
                    :class="{
                      configured: p.id !== 'custom' && data.profiles[p.id],
                      active: data.active === p.id
                    }"
                    @click="pick(p)"
                  >
                    <span class="ai-card-name">{{
                      p.id === 'custom' ? 'Añadir otro servidor…' : p.name
                    }}</span>
                    ...
                    <span v-if="p.id !== 'custom' && data.profiles[p.id]" class="ai-card-mark">
                      <Icon n="check" :t="12" />
                      {{ data.active === p.id ? 'en uso' : 'configurado' }}</span
                    >
                  </button>
                </template>
              </div>
            ...
            <div v-if="!groups.length" class="model-empty">
              ningún proveedor se llama así. Elige «Añadir otro servidor» si tienes una URL
              compatible con OpenAI.
            </div>
          </div>
          <div v-if="error && step === 'pick'" class="key-state bad">
            <Icon n="warning" :t="14" /> {{ error }}
          </div>
          <div v-if="data" class="ai-catalog-line">
            <span
              >Catálogo de modelos (models.dev): {{ data.catalog_status.models }} modelos de
              {{ data.catalog_status.providers }} proveedores · comprobado
              {{ formatAgo(data.catalog_status.checked_at) }}
              <span v-if="data.catalog_status.error" style="color: var(--amber)">
                · {{ data.catalog_status.error }}</span
              ></span
            >
            <button
              class="btn mini"
              type="button"
              :disabled="data.catalog_status.refreshing"
              @click="load(true)"
            >
              <Icon n="refresh" :t="12" /> Actualizar
            </button>
          </div>
        </template>
        ...
        <template v-else-if="chosen">
          <h3 class="modal-title">
            <button class="btn mini" type="button" title="Volver a la lista" @click="back">
              <Icon n="back" :t="13" />
            </button>
            ...
            <button
              ...
              type="button"
              title="Documentación"
              @click="openLink(chosen.docs)"
            >
              <Icon n="external" :t="12" /> docs
            </button>
            <button class="btn mini ai-close" type="button" title="Cerrar" @click="emit('close')">
              <Icon n="close" :t="13" />
            </button>
          ...
          <div class="ai-form">
            ...
            <div v-if="chosen.key !== 'none'" class="ai-key">
              <TextField
                ...
                :placeholder="
                  savedProfile?.has_key
                    ? 'guardada: ' + savedProfile.key + ' (escribe otra para cambiarla)'
                    : 'pega aquí la clave'
                "
              />
              <button
                v-if="chosen.key_url"
                class="btn mini ai-key-link"
                type="button"
                @click="openLink(chosen.key_url)"
              >
                <Icon n="key" :t="12" /> Consigue tu clave <Icon n="external" :t="11" />
              </button>
            </div>
            ...
            <TextField
              ...
              :hint="
                chosen.group === 'local' ? 'cambia el puerto si arrancaste el servidor en otro' : ''
              "
            />
            ...
            <button type="button" class="ai-advanced-toggle" @click="advanced = !advanced">
              <Icon n="right" :t="12" class="group-chevron" :class="{ open: advanced }" /> Avanzado
            </button>
            ...
            <div v-if="checkResult" class="key-state" :class="checkResult.ok ? 'ok' : 'bad'">
              ...
              <span v-if="checkResult.ok">
                ...
                <span v-if="checkResult.tools_ok">
                  · el asistente puede usar {{ checkResult.chat_model }}</span
                >
                <span v-else style="color: var(--amber)">
                  · {{ checkResult.chat_model }}: {{ checkResult.tools_reason }}</span
                >
              </span>

...
const props = defineProps({
  /** la ficha completa de la cancion (la de `api.song`), o null */
  song: { type: Object, default: null },
  /** hay una IA configurada y lista */
  aiReady: Boolean
})
...
const FIELD_LABELS = { album: 'álbum', year: 'año', genre: 'género', key: 'tono' }
...
const plainLyrics = computed(
  () => (props.song?.lyrics_synced ? props.song.lyrics : stripLrc(props.song?.lyrics)) || ''
)
...
const missingInfo = computed(() =>
  props.song ? Object.keys(FIELD_LABELS).filter((k) => !String(props.song[k] ?? '').trim()) : []
)
// Cuando la IA no puede completar, el boton no desaparece: se queda en ambar
// y deshabilitado, con el motivo debajo.
const blocked = ref(null)
const missingText = computed(() => missingInfo.value.map((k) => FIELD_LABELS[k]).join(', '))
...
// Cada peticion sabe de que cancion era. Las respuestas llegan cuando llegan:
// los detalles de IA o la transposicion de A se pintaban en la ficha de B si
// entre medias se elegia B. Lo que llega tarde para otra cancion no se pinta.
const isCurrent = (id) => props.song?.id === id

// ---------------------------------------------------------------- edicion
// Todo lo editable se guarda DENTRO del mp3 (etiquetas ID3), no en la base:
// la base es solo un indice reconstruible.
const EDITABLE = ['title', 'artist', 'album', 'year', 'genre', 'feat', 'key', 'bpm', 'lyrics']
...
const changes = computed(() => {
  ...
  return Object.fromEntries(
    EDITABLE.filter((k) => (draft.value[k] ?? '') !== base[k]).map((k) => [k, draft.value[k] ?? ''])
  )
})
...
async function changeCover() {
  ...
  if (!path || !isCurrent(id)) return
  loading.value = 'cover'
  failure.value = ''
  try {
    const r = await api.setCover(id, path)
    emit('updated', r.song)
    if (isCurrent(id)) coverVersion.value++
    notify(`Carátula cambiada (${r.kb} KB)`, 'ok')
  } catch (e) {
    if (isCurrent(id)) failure.value = errorMessage(e)
  } finally {
...
async function autofill() {
  ...
  const id = props.song.id
  loading.value = 'autofill'
  failure.value = ''
  blocked.value = null
  try {
    const r = await api.autofill(id)
    if (r.song) emit('updated', r.song)
    if (r.missing?.length && isCurrent(id)) {
      blocked.value = { reason: r.reason || 'La IA no pudo completar toda la ficha.' }
    }
    const n = Object.keys(r.filled || {}).length
    notify(
      n
        ? `Ficha completada: ${Object.keys(r.filled)
            .map((k) => FIELD_LABELS[k] || k)
            .join(', ')}`
        : r.reason || 'No se pudo completar nada',
      n ? 'ok' : 'info'
    )
  } catch (e) {
    if (isCurrent(id)) failure.value = errorMessage(e)
...
async function loadDetails() {
  const id = props.song.id
  loading.value = 'details'
  failure.value = ''
  try {
    const r = await api.details(id)
...
async function enrich(opts) {
  ...
  try {
    const r = await api.enrich(id, opts)
    // a la app siempre: la lista y la cola se ponen al dia; la ficha solo si
    // sigue siendo la de esta cancion (eso lo decide la app)
    emit('updated', r.song)
    const here = isCurrent(id)
    if (opts.cover && here) coverVersion.value++ // obliga a pedirla otra vez
    const hecho = []
    if (opts.lyrics)
      hecho.push(
        r.song?.lyrics && !antes.lyrics
          ? 'letra encontrada'
          : r.song?.lyrics
            ? 'ya tenia letra'
    ...
    // con otra cancion delante, se dice de cual era
    if (hecho.length)
      notify(
        (here ? '' : `«${name}»: `) + hecho.join(' · '),
...
<template>
  <aside v-if="song" class="details">
    <div class="details-head">
      <div class="cover-wrap">
        ...
        <button
          type="button"
          class="cover-blur"
          :title="song.blur ? 'Ver la portada como es' : 'Difuminar esta portada'"
        ...
        <button
          ...
          title="Elegir una imagen del disco"
          @click="changeCover"
        >
          <Icon n="image" :t="15" />
      ...
      <div class="details-titlebar">
        <div style="flex: 1; min-width: 0">
          <div class="copiable">
            ...
            <CopyButton
              :text="song.title || song.file"
              what="el título"
              @copied="
                (ok) => notify(ok ? 'Título copiado' : 'No se pudo copiar', ok ? 'ok' : 'info')
              "
            />
          </div>
          <div class="copiable">
            ...
            <CopyButton
              ...
              what="el artista"
              @copied="
                (ok) => notify(ok ? 'Artista copiado' : 'No se pudo copiar', ok ? 'ok' : 'info')
              "
            />
          </div>
        ...
        <button
          v-if="!editing"
          class="icon-btn"
          title="Editar la ficha (se guarda en el mp3)"
          @click="startEdit"
        >
          <Icon n="pencil" :t="14" />
        </button>
      ...
      <div style="margin-top: 10px; display: flex; align-items: center; gap: 12px">
        ...
        >
          <Icon :n="song.favorite ? 'heartFull' : 'heart'" :t="19" />
        </button>
      </div>
    </div>
    ...
    <dl v-if="!editing" class="facts">
      ...
      <dd>{{ song.genre || '—' }}</dd>
      <dt>Duración</dt>
      <dd class="mono">{{ formatDuration(song.duration) }}</dd>
      <dt>Calidad</dt>
      <dd class="mono">{{ song.bitrate ? Math.round(song.bitrate / 1000) + ' kbps' : '—' }}</dd>
      <dt>Tono</dt>
      <dd class="mono">{{ song.key || '—' }}</dd>
      <dt>BPM</dt>
      <dd class="mono">{{ song.bpm ? Math.round(song.bpm) : '—' }}</dd>
      <dt>Carpeta</dt>
      <dd :title="song.folder">{{ song.folder }}</dd>
    ...
    <div class="section">
      <div class="btn-row">
        <button
          ...
          class="btn mini"
          :disabled="!!loading"
          title="Se busca en LRCLIB; si no está, la completa la IA"
    ...
    <div v-if="song.lyrics || song.lyrics_synced" class="section copiable-section">
      <h4>
        Letra
        <CopyButton
          :text="plainLyrics"
          what="la letra"
          :size="14"
          @copied="(ok) => notify(ok ? 'Letra copiada' : 'No se pudo copiar', ok ? 'ok' : 'info')"
        />
        <button
          class="btn mini"
          type="button"
          title="La letra en grande, en una ventana aparte para el proyector"
          style="margin-left: auto"
          @click="projection.show()"
        >
          <Icon n="tv" :t="12" /> Proyectar
        </button>
        <button
          v-if="lrcLines"
          class="btn mini lrc-toggle"
          type="button"
          :class="{ on: follow }"
          :title="
            isPlayingThis
              ? 'La línea que suena, resaltada; pulsa una para ir ahí'
              : 'Con tiempos: al ponerla a sonar, sigue la letra'
          "
          @click="follow = !follow"
        >
          <Icon n="play" :t="11" /> {{ follow ? 'Siguiendo' : 'Seguir la canción' }}
        </button>
      </h4>
      <SyncedLyrics
        v-if="lrcLines && follow"
        :lines="lrcLines"
  ...
  <aside v-else class="details">
    <EmptyState
      icon="note"
      title="Selecciona una canción"
      hint="Aquí verás su ficha, la letra y los acordes"

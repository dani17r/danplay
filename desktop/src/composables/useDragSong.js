...
function setOver (el, x, y) {
  ...
  if (!target) { drag.after = false; return }
  const r = target.getBoundingClientRect()
  drag.after = target.getAttribute('data-drop-axis') === 'x'
    ? x > r.left + r.width / 2
    : y > r.top + r.height / 2
}
...
function trackScroller (el) {
  ...
  if (scroller && typeof requestAnimationFrame === 'function') frame = requestAnimationFrame(autoScroll)
}

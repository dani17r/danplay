...
export const FIELDS = [
  ...
  { k: 'muted', n: 'Texto secundario', d: 'artista, álbum, notas' },
 ...
 */
export const SIZES = {
  small: { name: 'Pequeño', scale: 0.9, note: 'cabe más en pantalla' },
  medium: { name: 'Mediano', scale: 1, note: 'el de siempre' },
  large: { name: 'Grande', scale: 1.14, note: 'letra más grande' },
  xlarge: { name: 'Muy grande', scale: 1.3, note: 'maxima comodidad' }
}

export const DENSITIES = {
  comfortable: { name: 'Cómoda', rowPad: '7px', fontSize: '13px' },
...
export function applyTheme(key, preview = null) {
  ...
  if (!preview) {
    try {
      localStorage.setItem('danplay.theme', key)
    } catch {
...
export function applyDensity(key) {
  ...
  try {
    localStorage.setItem('danplay.density', key)
  } catch {
    /* sin almacenamiento (modo privado) */
  }
  return d
...
export function applySize(key) {
  ...
  try {
    localStorage.setItem('danplay.size', key)
  } catch {
    /* sin almacenamiento (modo privado) */
  }
  return s
...
export function savedDensity() {
  try {
    return localStorage.getItem('danplay.density') || 'comfortable'
  } catch {
    return 'comfortable'
  }
}
